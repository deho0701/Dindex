...
template <class key_t, class val_t>
inline result_t HGroup<key_t, val_t>::get(const key_t &key, val_t &val) { // 알고리즘 9
  bool res = false;
  size_t pos = predict(key);

  if (data_array[pos].key == key) res = data_array[pos].val.read(val);  // line 4

  if (!res && data_array[pos].chain != nullptr)   //line 5 : bucket 에 해당데이터가 없을 경우 
    res = data_array[pos].chain->get(key, val);   //chain 에서 get

  if (!res && group_tmp)   // line 7 : bucket이  uninitialized, group_tmp(임시 그룹)가 비어있지 않은 경우
    res = group_tmp->get(key, val) == result_t::ok ? true : false;//group_tmp 에서 get
...
inline result_t HGroup<key_t, val_t>::put(const key_t &key, const val_t &val) {
  bool update_only = frozen; //false : insert(O)
  size_t pos = predict(key);

  if (upsert_bucket(key, val, update_only, pos)) { //upsert : 이전 버전이 존재하는경우 in-place, 없으면 insert
    return result_t::ok;
  }

  if (!data_array[pos].chain) { //lock 살펴보기
  ...
  if (!update_only) {   //update_only =true : only perform in-place  update to bucket and chain
    data_array[pos].chain->insert(key, val);  //in-place to chain
    return result_t::ok;
  }

  //in-place 실패시 
  if (!group_tmp) return result_t::retry;//group_tmp is null-> retry

  return group_tmp->put(key, val);  //in-place to bucket
}
...
template <class key_t, class val_t>
inline bool HGroup<key_t, val_t>::upsert_bucket(const key_t &key,   //in-palce or 새로운값 insert
                                                const val_t &val,
                                                bool update_only, size_t pos) {  //알고리즘 5 -update_record와 유사
  if (data_array[pos].key == 0) {  //새로운 값insert
    data_array[pos].val.lock();
  ...
  if (data_array[pos].key == key) {   // in-place
...
template <class key_t, class val_t>
inline void HGroup<key_t, val_t>::compact_phase_2() {  //포인터 치환 
  for (size_t i = 0; i < array_size; i++) {
